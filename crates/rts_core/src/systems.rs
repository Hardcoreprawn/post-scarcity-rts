...
// ============================================================================
// Resistance-Based Combat (New System)
// ============================================================================

/// Extended combat stats for resistance-based damage calculation.
///
/// This wraps the base CombatStats with additional resistance parameters.
#[derive(Debug, Clone, Copy)]
pub struct ResistanceCombatStats {
    /// Base weapon stats.
    pub weapon: crate::combat::WeaponStats,
    /// Resistance stats for damage reduction.
    pub resistance: crate::combat::ResistanceStats,
    /// Attack range.
    pub range: Fixed,
...
impl ResistanceCombatStats {
    /// Create from existing CombatStats (migration helper).
    #[must_use]
    pub fn from_legacy(stats: &CombatStats) -> Self {
        use crate::combat::{
            convert_flat_armor_to_resistance, ArmorClass, ExtendedDamageType, ResistanceStats,
            WeaponSize, WeaponStats,
        };

        let damage_type = ExtendedDamageType::from_damage_type(stats.damage_type);
        let armor_class = ArmorClass::from_armor_type(stats.armor_type);
        let resistance = convert_flat_armor_to_resistance(stats.armor_value, armor_class);

        Self {
            weapon: WeaponStats::new(stats.damage, damage_type).with_size(WeaponSize::Medium),
            resistance: ResistanceStats::new(armor_class, resistance),
            range: stats.range,
...
///
/// Wrapper around the combat module's `calculate_resistance_damage` function
/// for easier integration into existing systems.
#[must_use]
pub fn calculate_resistance_based_damage(
    attacker: &ResistanceCombatStats,
    target: &ResistanceCombatStats,
) -> u32 {
    crate::combat::calculate_resistance_damage(&attacker.weapon, &target.resistance)
}
